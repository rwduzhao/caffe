...
#include "caffe/layers/lstm2_layer.hpp"
...
__device__ Dtype tanh(const Dtype x) {
  return Dtype(2) * sigmoid_gpu(Dtype(2) * x) - Dtype(1);
}
...
__global__ void ActivationForward(const int nthreads, const int H,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    gate[index] = d < 3*H ? sigmoid_gpu(pre_gate[index]) : tanh(pre_gate[index]);
  }
}

template <typename Dtype>
__global__ void LSTM2Forward(const int nthreads, const int H, const int t,
...
template <typename Dtype>
__global__ void LSTM2Backward(const int nthreads, const int H, const int t, 
...
template <typename Dtype>
void LSTM2Layer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int t = 0; t < T_; ++t) {
    ...
    CUDA_POST_KERNEL_CHECK;
    LSTM2Forward<Dtype><<<CAFFE_GET_BLOCKS(N_*H_), CAFFE_CUDA_NUM_THREADS>>>(
...
template <typename Dtype>
void LSTM2Layer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int t = T_-1; t >= 0; --t) {
    ...
    LSTM2Backward<Dtype><<<CAFFE_GET_BLOCKS(N_*H_), CAFFE_CUDA_NUM_THREADS>>>(
...
INSTANTIATE_LAYER_GPU_FUNCS(LSTM2Layer);
