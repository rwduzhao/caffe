...
__global__ void kernel_row_sum(const int num, const int channels, const Dtype* data, Dtype* channel_sum) {
...
void OneStepUnifiedForgetGateMemoryLayer<Dtype>::Forward_gpu(
  ...
  for (int t = 0; t < time_step_; ++t) {
    ...
    caffe_gpu_gemm(CblasNoTrans, CblasTrans, batch_size_, extra_dim_, 1 * 1, (Dtype)1.,
                   e_t_1, weight_e, (Dtype)1., unified_pre_o_t);

    // unified pre-gate to pre-gate
    const Dtype *unified_pre_do_t_cpu_data = unified_pre_gate_.cpu_data() + unified_pre_gate_.offset(t);
    for (int n = 0; n < batch_size_; ++n) {
      const Dtype value = unified_pre_do_t_cpu_data[n];
      caffe_gpu_set(hidden_dim_, value, pre_o_t + pre_gate_.offset(0, n, 0, 0));  //TODO
    }
...
//    const Dtype alpha = Dtype(1.0) / Dtype(hidden_dim_);
//    caffe_gpu_scale(unified_pre_gate_.count(), alpha, unified_pre_do_t, unified_pre_do_t);
  }
