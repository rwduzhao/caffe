...
void LSTM2Layer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void LSTM2Layer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void LSTM2Layer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void LSTM2Layer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int t = T_-1; t >= 0; --t) {
    ...
    for (int n = 0; n < N_; ++n) {
      ...
      if (clipping_threshold_ > Dtype(0.)) {
        caffe_cpu_bound(4*H_, pre_gate_diff_t, -clipping_threshold_,
...
INSTANTIATE_CLASS(LSTM2Layer);
REGISTER_LAYER_CLASS(LSTM2);
