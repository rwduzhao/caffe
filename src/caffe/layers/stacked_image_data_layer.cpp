...
StackedImageDataLayer<Dtype>::~StackedImageDataLayer<Dtype>() {
...
template <typename Dtype>
void StackedImageDataLayer<Dtype>::InternalThreadEntry() {
...
template <typename Dtype>
void StackedImageDataLayer<Dtype>::ReadSourceListToLines() {
...
template <typename Dtype>
void StackedImageDataLayer<Dtype>::SetDatumSize() {
  ...
  if (image_color == StackedImageDataParameter_ImageColor_GRAY)
    this->datum_channels_ = 1 * num_stack;
  else if (image_color == StackedImageDataParameter_ImageColor_RGB)
    this->datum_channels_ = 3 * num_stack;
  else
    LOG(FATAL) << "unsupported image color";

  this->datum_height_ = this->layer_param_.stacked_image_data_param().new_height();
  this->datum_width_ = this->layer_param_.stacked_image_data_param().new_width();

  this->datum_size_ = this->datum_channels_ * this->datum_height_ * this->datum_width_;
}

template <typename Dtype>
void StackedImageDataLayer<Dtype>::ShuffleLines() {
...
bool StackedImageDataLayer<Dtype>::ReadSourceToTop(const int lines_id, const int batch_item_id) {
  ...
  const string source_item_prefix = this->layer_param_.stacked_image_data_param().source_item_prefix();
