...
void GroupSoftmaxLayer<Dtype>::Forward_gpu(
  ...
  for (int group_id = 0; group_id < group_ids_vec_.size(); ++group_id) {
    ...
    softmax_bottom_vec.push_back(&softmax_bottom);

    Blob<Dtype> softmax_top(1, group_size, 1, 1);
    vector<Blob<Dtype>*> softmax_top_vec;
    softmax_top_vec.push_back(&softmax_top);

    softmax_layer_->SetUp(softmax_bottom_vec, softmax_top_vec);
    ...
    for (int index = 0; index < group_ids.size(); ++index) {
      Dtype* softmax_bottom_data = softmax_bottom.mutable_cpu_data();
    ...
    for (int index = 0; index < group_ids.size(); ++index) {
      const Dtype* softmax_top_data = softmax_top.mutable_cpu_data();
...
void GroupSoftmaxLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    for (int group_id = 0; group_id < group_ids_vec_.size(); ++group_id) {
      ...
      for (int index = 0; index < group_ids.size(); ++index) {
        Dtype* softmax_top_diff = softmax_top.mutable_cpu_diff();
      ...
      for (int index = 0; index < group_ids.size(); ++index) {
        const Dtype* softmax_bottom_diff = softmax_bottom.mutable_cpu_diff();
