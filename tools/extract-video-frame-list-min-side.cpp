...
DEFINE_string(frame_count_split, "-", "frame count split");
...
const string GetVideoName(const string file_name, const string split = "-") {
  ...
  if (split == "") {
    return file_name.substr(0, pos_b);
  } else {
...
void ReadVideoFramesTable(const string &frame_list_filename,
  ...
  ifstream infile(frame_list_filename.c_str());
  string frame_filename;
  while (infile >> frame_filename) {
    const string video_name = GetVideoName(frame_filename, FLAGS_frame_count_split);
